"""Causal forest uplift estimator for Phase 2 (econml CausalForestDML).

CausalForestDML is used (not LinearDML) because the DGP has heterogeneous
segment-level CATEs; a forest final stage is the standard nonparametric choice.
Fails loudly on import/fit errors — no silent causalml fallback.
"""
...
def fit_causal_forest(
    ...
    Y = train_df[OUTCOME_COL].to_numpy()
    ...
    try:
        ...
        raise RuntimeError(
            "CausalForestDML.fit failed. Per project policy we do not fall back "
            "to causalml. Inspect econml / sklearn compatibility.\n"
            f"Original error: {exc}"
        ) from exc
    return model, feats
